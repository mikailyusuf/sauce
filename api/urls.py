from django.urls import path

from api import views
from api.views import Logout, Login, Record, Tickets, OrderTicket, GetTickets, AddTicket, CreateToken

urlpatterns = [
    path('addUser/', Record.as_view(), name="register"),
    path('login/', Login.as_view(), name="login"),
    path('logout/', Logout.as_view(), name="logout"),
    path('get_tickets/',GetTickets.as_view(), name="logout"),
    path('add_ticket/', AddTicket.as_view(), name="logout"),
    path('order/', OrderTicket.as_view(), name="order"),
    path('list/<int:pk>', views.getUserTicket),
    path('createToken', CreateToken.as_view()),

]
